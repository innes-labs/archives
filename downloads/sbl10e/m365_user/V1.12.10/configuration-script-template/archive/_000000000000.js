...
// SBL10e m365_user V1.12.10 (or above) (.conf WebDAV directory)
...
function executeScript() {
	...
	setHostname("sbl-" + getPsn(PSN_ORDINAL)); 
	...
	// ---- Set the tenant id of your Azure AD application used for OAuth authentication: uncomment the line after
	//setDatasourceTenantId("00000000-0000-0000-0000-000000000000"); 
    setDatasourceTenantId("75ba04c6-bb4d-4c23-810a-d7b29b69720d"); 
	
	// ---- Set the client id of the Azure AD application used for OAuth authentication: uncomment the line after
	//setDatasourceClientId("00000000-0000-0000-0000-000000000000"); 
	setDatasourceClientId("50dacfa5-868c-46e6-9074-55f79d9d1c2b"); /* */ 

	// ---- Set the secret value of the Azure AD application used for OAuth authentication: uncomment the line after
	//setDatasourceClientSecret("my_AzureAD_App_secret_value"); 
	setDatasourceClientSecret("65hKR0Z2W+AHnWLlBKSbmnUNWSgOuE9fiW666alFc8g="); /* */
	
	// ---- Set the email value of the M365 delegate account: uncomment the line after
	//setDatasourceUsername("myDelegate@demo-contoso.onmicrosoft.com"); /* e.g. myDelegate@contoso.onmicrosoft.com */
	setDatasourceUsername("sbl10e-m365_user-delegate@innes.fr"); 

	// ---- Set the password value of the M365 delegate account: uncomment the line after
	//setDatasourcePassword("myDelegatePwd1"); /* e.g. myDelegatePwd1 */
	setDatasourcePassword("as$Kd3uZ6@46S=C");

	// ---- Set the email value of the user account from which the calendar availability must be controled: uncomment the line after
	//setDatasourceResourceId("employee1@demo-contoso.onmicrosoft.com"); 
	setDatasourceResourceId("frederic.inizan@innes.fr"); /* e.g. employee1@contoso.onmicrosoft.com */ 

	// ---- Set the datasource heartbeat: uncomment the line after
	//setDatasourceHeartbeat(60); // duration in seconds between two following connections to the M365 user calendar 
	setDatasourceHeartbeat(59);
	...
	//enableProxy(); // ---- This one to enable Proxy - Proxy server is not supported in this version
	//disableProxy(); // ---- This one to disable Proxy
	...
	setRebootTime(15, 28); // 1st param: value for hour from 0 to 23, 2th param: value for minute from 0 to 59 (in the example, 6.00 AM)
	
	// ---------------------------------------
	// ---- Maintenance > Preferences (CORS)
	// ---------------------------------------
	// Enable/disable CORS (Cross-origin resource sharings) request on SBL10e device (suitable for Web Applications calling SAP10e Web services from Web browsers): uncomment one of the two lines after
...
function setRebootTime(hours, minutes) {
 ...
 function setDatasourceTenantId(tenant_id) {
	setStringPref("appli.network.datasource.credential.oauth_password_grant.tenant_id", tenant_id);
}
function setDatasourceClientId(client_id) {
	setStringPref("appli.network.datasource.credential.oauth_password_grant.client_id", client_id);
}
function setDatasourceClientSecret(client_secret) {
	setStringPref("appli.network.datasource.credential.oauth_password_grant.client_secret", client_secret);
}
function setDatasourceUsername(username) {
	setStringPref("appli.network.datasource.credential.oauth_password_grant.username", username);
}
function setDatasourcePassword(password) {
	setStringPref("appli.network.datasource.credential.oauth_password_grant.password", password);
}
function setDatasourceResourceId(resource_id) {
	setStringPref("appli.network.datasource.ressource.id", resource_id);
}
