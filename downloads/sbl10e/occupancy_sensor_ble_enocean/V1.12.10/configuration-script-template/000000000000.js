...
// SBL10e occupancy_sensor_ble_enocean V1.12.10 (or above) (.conf WebDAV directory)
...
function executeScript() {
	...
	// ---------------------------------------
	// --- Configuration > Sensors pairing ---
	// ---------------------------------------

	// ---- Set the paired EnOcean BLE sensor: uncomment the two lines after
	//var macAddress = "e5:00:00:00:25:18"; // EnOcean sensor MAC address, starting with e5:00, in lower case with double dot (:) separator (ex: "e5:00:00:00:25:18") 
	//setPairedEnOceanBLESensorDevice(macAddress);	
	...
	//enableProxy(); // ---- This one to enable Proxy server - Proxy server is not supported in this version
...
function verifyScriptTarget() {
	...
	if (getFirmwareApp() != "occupancy_sensor_ble_enocean") {
		return false;
	}
	return true;
}
...
function setPairedEnOceanBLESensorDevice(macAddr) {
	setStringPref("appli.sensor.1.mac", macAddr);
}
