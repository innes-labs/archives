 ...
 *
 * Define complete user process
 *
 * ADD ACTION for correct user lookup (Requea)
 *
 * Add setting GPIO (see MeetingControlSM.js)
 *
 * Remove unwanted code
 *
 * Define statemachine to implement user's process
 *
 * Version: 1.13.10
 */
...
(function () {
	...
	function userAuthenticate(event) {
		...
		window.logger.debug("MeetingControlDoor : userAuthenticate : EV : " + event);

		window.logger.debug("MeetingControlDoor : userAuthenticate : EV : " + event.detail);
		// event.detail is null
		var msg = "open";
		//info2
		if (event.detail) {
			msg = event.detail.info1;

			badgeEmail = msg;
		}


		window.logger.debug("MeetingControlDoor : userAuthenticate : " + msg);

		SignmeetingCommons.Tools.dispatchEvent("authRefused", "");

		SignmeetingCommons.Tools.dispatchEvent("addCustomButton", msg);
	...
	function checkOrganizer() {
		try {
			...
			var currentMeeting = hostServices.parent.getCacheItem(currentMeetingId);
			// this.getUserInfo(this.user,false)
			window.logger.debug("Statemachine : checkOrganizer : " + currentMeeting);
			if (currentMeeting) {
				window.logger.debug("Statemachine : checkOrganizer : " + currentMeeting.organizer.commonName);
				var user = currentMeeting.organizer.commonName;
				$.when(
					hostServices.getUserInfo(user, true))
					.then(
						function (item) {
							window.logger.debug("Statemachine : ############################ ===> checkOrganizer " + item);
	...
	function confirmCurrent() {
		...
		try{
		 isServerActionActive = true;
		 var itemToConfirm = hostServices.parent.getCacheItem(currentMeetingId);

			 $.when(
					hostServices.confirmItem(itemToConfirm))
				.then(
						function(item){
